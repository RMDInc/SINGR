...
private: System::Windows::Forms::TextBox^  tb_TX_file_size;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->tb_TX_file_size = (gcnew System::Windows::Forms::TextBox());
			...
			// 
			this->diagnosticsToolStripMenuItem->DropDownItems->AddRange(gcnew cli::array< System::Windows::Forms::ToolStripItem^  >(2) {
				this->captureWaveformsToolStripMenuItem,
					this->recoverMMToolStripMenuItem
			});
			...
			this->recoverMMToolStripMenuItem->Size = System::Drawing::Size(179, 22);
			...
			// progbar_TX
			// 
			this->progbar_TX->Location = System::Drawing::Point(1157, 460);
			...
			this->label11->Location = System::Drawing::Point(1061, 465);
			...
			// tb_TX_file_size
			// 
			this->tb_TX_file_size->BackColor = System::Drawing::SystemColors::Control;
			this->tb_TX_file_size->Location = System::Drawing::Point(1061, 518);
			this->tb_TX_file_size->Name = L"tb_TX_file_size";
			this->tb_TX_file_size->Size = System::Drawing::Size(196, 20);
			this->tb_TX_file_size->TabIndex = 65;
			// 
			// b_etherConnect
			// 
			...
			this->Controls->Add(this->tb_TX_file_size);
...
//check/open the serial connection to call the TX function
	...
	//this->tb_updates->Text = "Fetching file size...";
	...
	//{
	//	this->tb_updates->Text = "Timed out getting the file size. Try again in a moment.";
	//	this->b_transferDataFile->Text = "Transfer SD Data File";
	//	tx_file.close();
	//	return;
	//}

	//new approach, don't get value from the board, just use the size of the file that we received already
	m_TX_file_size = g_RECV_file_size;
	...
	else if (m_TX_file_size == 0)
	{
		this->tb_updates->Text = "File on the board is empty.";
		this->b_transferDataFile->Text = "Transfer SD Data File";
		tx_file.close();
		return;
	}

	//determine how many steps we will take and how often we need to do that based on the file size
	m_TX_steps_to_take = 100 / (this->progbar_TX->Step);
	m_TX_step_size = m_TX_file_size / m_TX_steps_to_take;
	this->tb_TX_file_size->Text = "Total size to transfer: " + m_TX_file_size + " bytes";
	...
	while (m_running)
	{
		...
		if (iResult > 0)
		{
			...
			{
				m_TX_steps++;
				this->progbar_TX->PerformStep();	//performs a step //steps are 1% currently
				Application::DoEvents();
			}
